...
@testset "rmsc04 core functions" begin
    ...
    # - L2 times
    L2_times_benchmark = CSV.read("./benchmarks/rmsc04/L2_times.csv", DataFrame)[:, 2:end] |> Array{Union{Missing, Float64}};
    @test sum(L2.times .== L2_times_benchmark) == length(L2.times);

    # - L2 bids
    L2_bids_p_benchmark = CSV.read("./benchmarks/rmsc04/L2_bids_p.csv", DataFrame)[:, 2:end] |> Array{Union{Missing, Float64}};
    L2_bids_q_benchmark = CSV.read("./benchmarks/rmsc04/L2_bids_q.csv", DataFrame)[:, 2:end] |> Array{Union{Missing, Float64}};
    @test L2.bids[:,:,1] == L2_bids_p_benchmark;
    @test L2.bids[:,:,2] == L2_bids_q_benchmark;

    # - L2 asks
    L2_asks_p_benchmark = CSV.read("./benchmarks/rmsc04/L2_asks_p.csv", DataFrame)[:, 2:end] |> Array{Union{Missing, Float64}};
